...
function workspaceWindowTitle(state, companion) {
  if (companion) return `NATION — Connected to: ${companion.serverName} (${new URL(companion.endpoint).host})`;
  const active = activeEnvironment(state);
  return active ? `NATION — Hosted: ${active.name} (${new URL(active.origin).host})` : "NATION";
}
