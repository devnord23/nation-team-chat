...
export function createManagedDesktopClient({ store, applyConnection, openBrowser, platform, deviceName,
  ...
  function clearConnection() {
    ...
    const operation = (async () => {
      ...
      const warnings = [];
      if (runtime.status === "rejected") warnings.push("The local runtime did not confirm stopping Company tasks. Quit and reopen NATION before using Company models again.");
  ...
  async function endAccess(stamp, message) {
    ...
    catch { message += " Quit and reopen NATION to confirm Company tasks have stopped."; }
  ...
  return {
    ...
    async start() {
      ...
      catch { return current(stamp) ? publish({ status: "unavailable", message: "Company sign-in could not be restored. Unlock your system keychain and restart NATION." }) : snapshot(); }
      return refresh();
    },
    async begin(input) {
