...
// paths without registering them, so they do not count. With accounts on, the
// account gateway answers email sign-in before either.
...
const serverSource = [join(ROOT, "server", "index.ts"), join(ROOT, "server", "account-gateway.ts"), ...routeModules.map((name) => join(ROUTES_DIR, name))]
