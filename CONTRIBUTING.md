...
- stable `nation-amd64.deb and nation.AppImage` copies used by the latest-download links;
...
Data lives in `~/.nation/` (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real `~/.nation`.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in `~/.nation/config.json` via `PUT /api/config` and the API
