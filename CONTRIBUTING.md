...
```sh
git clone https://github.com/devnord23/nation-team-chat && cd nation-team-chat
pnpm install
...
- stable the configured stable package filenames copies used by the latest-download links;
...
[Nation Team Chat releases](https://github.com/devnord23/nation-team-chat/releases). Then verify the checksum
...
Data lives in the configured data directory (bots, transcripts, per-thread NDJSON event logs, config with keys).
...
- **Never touch the real data directory.** The setup file points `HOME` at a temp dir; keep it
  that way.
...
API keys are write-only: they land in the configured data-directory config file via `PUT /api/config` and the API
...
Nation Team Chat repository, and never change the upstream publish target in a feature PR unless
