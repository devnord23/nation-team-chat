...
const SURFACE_AUTHORITY =
  " For browser and computer tasks, use Nation Team Chat's mounted browser/computer tools first: inspect the target, perform the action, and verify its result before claiming success. Discover deferred tools by their server/name when needed. Do not substitute the provider's own desktop, a shell-launched browser, or another automation path for the selected Nation Team Chat surface. Ordinary code and file tasks may still use their normal tools. Announcing an action is not performing it. A request naming another place does not move these tools: this computer is the user's host, Local VM is an isolated desktop, the cloud computer is remote, and the built-in browser is a separate browser. If the requested place differs from the mounted one, explain the mismatch and ask the user to change the conversation's computer selector; never act on a different computer or describe a host window as a VM.";
...
export function surfacePrompt(
  ...
  if (text) text += RESTATE_SENTENCE + (opts.canSelect
    ? SURFACE_AUTHORITY.replace("explain the mismatch and ask the user to change the conversation's computer selector", "inspect connected choices with select_computer and select the requested available place; on a pending result end this turn so Nation Team Chat can reconnect the correct tools and continue the original request")
    : SURFACE_AUTHORITY);
  else if (!opts.note && !opts.canSelect) {
    text = " No computer or built-in browser tools are mounted this turn. You cannot open apps, click, or inspect a screen through Nation Team Chat. If asked for screen work, explain this and ask the user to choose and connect a computer in the Computer panel; do not claim to have opened or checked it.";
  }
  if (opts.pinned) {
