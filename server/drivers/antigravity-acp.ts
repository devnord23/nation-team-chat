...
export async function prepareAntigravityProfile(input: {
  ...
  if (/\r|\n|\0|%s/u.test(executable) || executable.includes(delimiter)) {
    throw new Error("The NATION runtime path cannot safely suppress Antigravity browser launches.");
  }
      ...
      const timer = setTimeout(() => {
        ...
        if (method === "initialize") {
          ...
          reject(new Error(
            ...
            "Retry setup. If it still fails, share this error and your NATION version; do not paste Google sign-in links or tokens.",
          ));
