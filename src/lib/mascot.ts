...
export type NationState = CursorState;
export const NATION_STATES = CURSOR_STATES;
...
} satisfies Record<string, NationState[]>;

export const NATION_COLOR_NAMES = [
  "green",
  "blue",
  "red",
  "orange",
...
export type NationColor = (typeof NATION_COLOR_NAMES)[number];
...
export const NATION_COLORS = {
  ...
  teal: "#01A492",
  coral: "#E5634E",
} satisfies Record<NationColor, string>;

export const NATION_MOTIONS = [
  "arrive",
  "switch",
  "customize",
  "alert",
...
export type NationMotion = "none" | (typeof NATION_MOTIONS)[number];
...
const KNOWN_STATES = new Set<string>(NATION_STATES);

/** Resolves any stored value — current, legacy or junk — to a real state. */
export function normalizeState(value: string | null | undefined): NationState | null {
  if (!value) return null;
  if (KNOWN_STATES.has(value)) return value as NationState;
 ...
 */
export const PICKABLE_STATES: NationState[] = [
  "idle", // expression 0
  "happy", // 2
  "curious", // 3
  "drowsy", // 4
 ...
 */
export function stateForBot(bot: MascotBotProfile): NationState {
