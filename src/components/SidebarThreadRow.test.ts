...
describe("Copy link", () => {
  ...
  it("writes the exact canonical link for the row's owner to the clipboard", () => {
    ...
    const rows = [
      { task: { threadId: "qa-245", title: "QA PR 245" }, ownerId: "scout", link: "nation://thread/qa-245?bot=scout" },
      { task: { threadId: "monday-1", title: "Monday plan" }, ownerId: "standup", link: "nation://thread/monday-1?bot=standup" },
    ];
