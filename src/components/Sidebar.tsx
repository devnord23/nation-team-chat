    ...
    >
      ...
      <div className={cn("pb-3 pt-2", density === "icons" ? "px-2" : "px-3")}>
        {density === "icons" && (
          ...
          {/* Everyone connects their own apps; owner-only parts stay inside the panel. */}<button data-tour="nav-apps" onClick={() => dispatch({ type: "togglePlugins", open: true })} aria-label={t("sidebar.nav.connectedApps")} title={t("sidebar.nav.connectedApps")} className={cn("flex min-h-10 w-full items-center rounded-xl py-2 hover:bg-raised/50", density === "icons" ? "justify-center px-2" : "gap-3 px-3")}><Puzzle size={20} className="text-ink-secondary" /><span className={cn("text-[14px] text-ink", density === "icons" && "hidden")}>{t("sidebar.nav.connectedApps")}</span></button>
          </>
        )}
        {density === "icons" && (
          <SidebarPhoneButton
          ...
          <SidebarMoreMenu
            items={[
              ...
              { key: "plugins", tourId: "nav-apps", label: t("sidebar.nav.connectedApps"), icon: <Puzzle size={18} />, onSelect: () => dispatch({ type: "togglePlugins", open: true }) },
            ]}
          />
        )}
        {density === "icons" ? (
