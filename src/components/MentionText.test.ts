...
import { NATION_COLORS } from "@/lib/mascot";
...
it("uses each bot's avatar palette and updates when its color changes", () => {
  for (const color of ["red", "blue"] as const) {
    ...
    expect(html).toContain(`style="--mention-color:${NATION_COLORS[color]}">@Atlas`);
    expect(html).toContain(`style="--mention-color:${NATION_COLORS.purple}">@Juniper`);
