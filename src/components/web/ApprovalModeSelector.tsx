...
/** Hosted API approvals use the two modes supported over the web. */
export function approvalModeOptionsFor(_driverKind: string, _trustedModesAvailable = true) {
  return approvalModeOptions()
    .filter(option => option.mode === "ask" || option.mode === "auto")
    .map(option => option.mode === "auto" ? { ...option, description: t("approvalMode.noNativeReview") } : option);
}
