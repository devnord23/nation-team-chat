...
const RELEASE_REPOSITORIES = [
  'devnord23/nation-team-chat',
] as const;
const RELEASES_PER_PAGE = 100;
const MAX_RELEASE_PAGES = 10;
const LEGACY_DRAFT_NOTES =
  /^Draft assembled by the release workflow from devnord23\/nation-team-chat@([0-9a-f]{40})\. Edit these notes, then publish\.\s*$/i;
...
function releaseNotes(release: GitHubRelease) {
  ...
  return `This build predates curated release notes. [View its source commit (${commit.slice(0, 7)})](https://github.com/devnord23/nation-team-chat/commit/${commit}).`;
}
    ...
    for (let page = 1; page <= MAX_RELEASE_PAGES; page += 1) {
      ...
      const response = await fetch(endpoint, {
        headers: {
          ...
          'User-Agent: nation-team-chat-docs',
  ...
  if (releases.length === 0) {
    return (
      <p>
        ...
        <a href="https://github.com/devnord23/nation-team-chat/releases">Browse releases on GitHub</a>.
      </p>
    );
  }
